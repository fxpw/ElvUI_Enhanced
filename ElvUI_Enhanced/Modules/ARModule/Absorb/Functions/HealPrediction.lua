...
function ARModule.HealthClipFrame_HealComm(frame)
	if frame.HealCommBar then
		ARModule:SetAlpha_HealComm(frame.HealCommBar, 1)
	end
end

function ARModule:SetAlpha_HealComm(obj, alpha)
...
function ARModule:SetTexture_HealComm(obj, texture)
...
function ARModule:SetFrameLevel_HealComm(obj, level)
...
function ARModule:Construct_HealComm(frame)
	...
	local prediction = {
		...
		PostUpdate = ARModule.UpdateHealComm,
		maxOverflow = 1,
		health = health,
		parent = parent,
		frame = frame
	...
	ARModule:SetAlpha_HealComm(prediction, 0)
	ARModule:SetFrameLevel_HealComm(prediction, 12)
	ARModule:SetTexture_HealComm(prediction, E.media.blankTex)
	dbUpdater(frame)
	return prediction
end

function ARModule:SetSize_HealComm(frame)
	local health = frame.Health
...
function ARModule:Configure_HealComm(frame)
	...
	if not db.anchorPoint then
		dbUpdater(frame)
		if db.enable ~= E.db.enhanced.unitframe.units[frame.unitframeType].absorbPrediction.enable then db.enable = true end -- workaround because RaidGroup1Button1 and PartyGroup1Button1 db.enable were being rewritten to false for some reason
	end
	...
	if db and db.enable then
		...
		local unit = frame.unitframeType
		db = E.db.enhanced.unitframe.units[unit].absorbPrediction
		E:Delay(1, dbUpdater, frame) -- workaround to db being overwritten after configure

		local colors = UF.db.colors.healPrediction
		local absorbColors = E.db.enhanced.unitframe.colors.absorbPrediction
		...
		ARModule:SetTexture_HealComm(pred, UF.db.colors.transparentHealth and E.media.blankTex or healthBarTexture:GetTexture())
		...
		if orientation == "HORIZONTAL" then
			...
			-- (well in this case, width) other than -1 which positions the ARModule on the left or right side
...
function ARModule:UpdateHealComm(_, myIncomingHeal, otherIncomingHeal, absorb, _, hasOverAbsorb, hasOverHealAbsorb, health, maxHealth)
	...
	if not pred.anchor then
		ARModule:Configure_HealComm(frame) -- workaround to db.enable returning false for some reason on configure_HealComm initial run
	end

	ARModule:SetSize_HealComm(frame)
	...
	-- color ARModule bar if in over state
